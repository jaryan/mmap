...
software languages. As \R straddles the line between outright
...
The new package \emph{mmap} by Ryan, allows for \R like syntax to be
with native operating system mmap calls.  The package aims to provide
a thin layer in \R around the underlying operating system`s mmap
functionality.  As such, it relies heavily on the system calls for
all functionality.  This increases the performance of the package
while minimizing the learning curve for developers.  At the same
time the \emph{mmap} package provides \R based abstraction
to make the mapped file access as transparent as possible.  The
downside to this thin wrapper is that \R performance is now
tied to the underlying OS implementation.  
...
with commentary as well as \R examples.  The paper will conclude
...
an \R work-flow.  This meant keeping the \R calls
very close to the semantics of the underlying system
calls, as well as provide an \R-like experience to the
end user.

Mapping a file that contains integer values via a system
call would return single byte char values.  This
behavior would of course be possible within \R, but wouldn't allow 
for the richness of the language to be exposed.  Instead
the design allows for automatic translation through a
collection of special \textff{Ctypes} that replicate
native \R and native machine atomic data types.

This combines expected \R results, with the expected OS
semantics.  The available system types even allow for
virtual support of types not expressable in \R, so as to
allow for better disk space management, as well as allow
for interoperability with non-\R programs.

A motivating reason to use memory-mapped files in general
is to simplify the interface to data residing on disk.
In \R, one would typically have to read in an entire file
to operate on a subset of it.  There are numerous packages
that make this less a problem, notably \emph{bigmemory} (Kane
...
as data residing in memory.  This reduces the need for
...
files as well.  This includes all the basic Ops functionality
as well as some basic tools to investigate the mmap structure
itself.
